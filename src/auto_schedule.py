...
while True:
    ...
    for j in jobs:
        ...
        if pid == 0:
            ...
            while schedulerInfo:
                while schedulerInfo['SchedulerTimeUntilStart'] > 0:
                    # Time remaining is in minutes, convert to seconds and wait
                    time.sleep(schedulerInfo['SchedulerTimeUntilStart'] * 60)
                ...
                # Sleep for interval
                time.sleep(schedulerInfo['SchedulerSecondsBetweenActivation'])

                # If end time has passed, kill child
                if schedulerInfo["SchedulerTimeUntilStop"] < 0:
                    os._exit(0)
        else:
            children.append(pid);
